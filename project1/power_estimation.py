...
duration_wifi_off = 0.182500 #[s]
duration_wifi_on = 0.195000
...
energy_wifi_off = (avg_power_wifi_off+avg_power_idle_sensor)*duration_wifi_off*1000
energy_wifi_on = (avg_power_wifi_on+avg_power_sensor_reading)*duration_sensor_reading*1000 +(avg_power_wifi_on+avg_power_idle_sensor)*(duration_wifi_on-duration_sensor_reading)*1000
...
energy_transmission = avg_power_wifi_on*duration_transmission

# Calculate the total energy consumption for one transmission cycle (in mJ)
total_energy_consumption = (energy_boot+energy_wifi_off+energy_wifi_on+energy_deep_sleep)/1000 #[J]
...
print(f"Total Energy Consumption for one Transmission Cycle: {total_energy_consumption} J")


######################## POINT 3:
#BATTERY_ENERGY:
Y = 18402 #1071(8402) [mJ]
number_cycle = Y/total_energy_consumption
print(number_cycle)
...
#2.il sensore rimane sempre alimentato e quindi consuma energia costanetemente, si potrebbe agigungere un interruttore in modo che si accedna solo quando wifi è on
